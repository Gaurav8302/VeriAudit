...
>
> This document is the product authority for VeriAudit. It is reproduced from
> `VeriAudit_SOURCE_OF_TRUTH.md` at the repository root without changes to its
> meaning. Where any other document in `docs/` disagrees with this one on
> *product intent*, this document wins.
>
> The one thing it defers on: `Do not invent CooL API capabilities. Use the
> actual installed SDK API.` (section 6). Where product intent and the real SDK
> collide, `COOL_SDK_AUDIT.md` decides what is technically possible and
> `SECURITY_AND_CLAIMS.md` decides what we are allowed to say.
...
History becomes large and the original work is buried
        â†“
A senior person asks: â€œWhy did the AI make this decision?â€
        â†“
...
Evidence loaded              âœ“
Documents parsed             âœ“
Relevant evidence retrieved  âœ“
Controls tested              âœ“
Finding generated             âœ“
Human verification requested âœ“
```
...
> **AI worked â†’ time passed â†’ the decision was questioned â†’ the history
> was buried â†’ VeriAudit reconstructed it â†’ the evidence was verified.**
